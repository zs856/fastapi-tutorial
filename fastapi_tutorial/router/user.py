...
def update_user(id: int, request:UserBase, db: Session=Depends(get_db), current_user: str= Depends(get_current_user)):
    return db_user.update_user(db,id, request)
# Delete user
@router.get('/delete/{id}')
def delete(id:int, db:Session=Depends(get_db), current_user: str= Depends(get_current_user)):
